...
export class BiomeView {
 ...
 build(g:Game){this.root.clear();this.ownedGeometry.forEach(x=>x.dispose());this.ownedMaterials.forEach(x=>x.dispose());this.ownedGeometry=[];this.ownedMaterials=[];const data=BOSSES[g.boss.index];
  ...
  const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(positions,3));geo.setAttribute('color',new T.Float32BufferAttribute(colors,3));const uv:number[]=[];for(let i=0;i<positions.length;i+=3)uv.push(positions[i]/12,positions[i+2]/12);geo.setAttribute('uv',new T.Float32BufferAttribute(uv,2));geo.setIndex(indices);geo.computeVertexNormals();this.ownedGeometry.push(geo);this.groundMaterial=new T.MeshStandardMaterial({map:this.assets.textures.get('ruin-stone'),vertexColors:true,roughness:.92,flatShading:true,side:T.DoubleSide});this.ownedMaterials.push(this.groundMaterial);const ground=new T.Mesh(geo,this.groundMaterial);ground.receiveShadow=true;this.root.add(ground);
